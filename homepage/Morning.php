...
$result = mysqli_query($conn, "SELECT * FROM Koreancar ORDER BY NAME");
?>
<!DOCTYPE html>
<html>
<head>
	...
	<body>
		...
		<div class="container">
			<div class="row">
				<ul class="list-group">
           <a href="Morning.php" class="list-group-item">
            <?php
            $row = mysqli_fetch_assoc($result);
            echo '<p>'.$row['Name'].'</p>';
            ?></a>
            <a href="Porter2.php" class="list-group-item">
            <?php
             $row = mysqli_fetch_assoc($result);
             echo '<p>'.$row['Name'].'</p>';
             ?></a>
            <a href="Tico.php" class="list-group-item">
            <?php
             $row = mysqli_fetch_assoc($result);
             echo '<p>'.$row['Name'].'</p>';
             ?></a>
			...
			<img class="media-object" src="images/Morning.jpg" alt="Morning 이미지">
      <?php
       $result = mysqli_query($conn, "SELECT * FROM koreancar WHERE koreancar.Name = 'Morning'");
       $row = mysqli_fetch_assoc($result);
       ...
       ?>
          ...
          <?php
          $result = mysqli_query($conn, "SELECT * FROM koreancarspec WHERE koreancarspec.Name = 'Morning'");
          $row = mysqli_fetch_assoc($result);
